...
class Base(DeclarativeBase):
    pass
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://posts_6pwa_user:Bm3fwHNtQhYVkkMszLw7ERR5xlRTdj1T@dpg-cpf9i4n109ks73bfqhqg-a/posts_6pwa'
...
# def authorised_user_only(f):
#     @wraps(f)
#     def decorated_function(*args, **kwargs):
#         post_id = kwargs.get('post_id') or args[0]
...
def my_blogs():
    if not current_user.is_authenticated:
        flash("You need to login to view your blogs.")
        return redirect(url_for('login'))
    
    user_posts = db.session.execute(db.select(BlogPost).where(BlogPost.author_id == current_user.id)).scalars().all()
    print("User Posts: ", user_posts)  # Debugging line
    return render_template("my_blogs.html", all_posts=user_posts, current_user=current_user)
...
def contact():
    if request.method == "POST":
        ...
        return render_template("contact.html", success=True, current_user=current_user)
    return render_template("contact.html", success=False, current_user=current_user)
...
def send_email(name,email):
    ...
    with smtplib.SMTP("smtp.gmail.com") as connection:
        ...
        connection.sendmail(OWN_EMAIL, email, msg.as_string())
